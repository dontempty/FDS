#!/usr/bin/env python3
"""
d_M* (HRR=863 kW) vs dq_M* (HRR=1097 kW) — T(z) averaged over [0, 300 s].

Output: analysis/grid_conv/d_dq/d_dq_300.png
"""
...
RES    = ROOT / "Results"
OUT    = ROOT / "analysis" / "grid_conv" / "d_dq"
...
FIRE_CENTER_Y = 1.45
WIN           = (0.0, 300.0)   # full run average

# dy offsets to show as subplot columns
DY_COLS = [
    (0.0, "Δy=0.0 (centerline)"),
    (0.5, "Δy=0.5 m"),
    (1.0, "Δy=1.0 m"),
]
...
def main():
    ...
    fig, axes = plt.subplots(1, len(DY_COLS), figsize=(5 * len(DY_COLS), 6), sharey=True)
    fig.suptitle(
        "d (HRR 863 kW, solid)  vs  dq (HRR 1097 kW, dashed)\n"
        f"T(z) averaged over [{WIN[0]:.0f}, {WIN[1]:.0f}] s  |  YZ SLCF @ x≈{SLICE_X:.1f} m",
        fontsize=11,
    )

    for ax, (dy, dy_lbl) in zip(axes, DY_COLS):
        ...
        ax.set_title(f"{dy_lbl}  (y={y_target:.2f} m)", fontsize=10)
        ax.set_xlim(0, 3.2)
