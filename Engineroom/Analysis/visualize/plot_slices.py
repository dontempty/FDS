...
RES  = ROOT / "Results"
OUT  = ROOT / "analysis" / "vis"
OUT.mkdir(parents=True, exist_ok=True)

WIN = (550.0, 600.0)   # quasi-steady fire phase
...
def plot_single_slice(ax, fig, sim, obsts, holes, ori, target, xlabel, ylabel,
    ...
    print(f"  ori={ori}: t_avg 550–{t_end:.0f} s  OK")
    return True
...
def plot_mesh(case_dir, mesh_lbl):
    ...
    for (ori, target, xlabel, ylabel, xlim, ylim, vmin, vmax, slc_lbl) in SLICES:
        ...
        fig.suptitle(
            f"{mesh_lbl}  |  {slc_lbl}\n"
            f"Temperature — time-averaged 550–600 s",
            fontsize=10,
        )
        tag = axis_tag[ori]
        axis_count[ori] += 1
    ...
    out_path = OUT / f"slices_fire600_{case_dir}.png"
    fig.savefig(out_path, dpi=150)
    plt.close(fig)
    print(f"  → {out_path}\n")
